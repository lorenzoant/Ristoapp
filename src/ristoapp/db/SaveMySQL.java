...
public class SaveMySQL {
	...
	public RistorantiBean getInfoRistoranteDaID(int ID) throws Exception{ // Tazza
		...
		try {
			...
			// Creo stringa sql
			String sql = "SELECT * FROM Ristoranti WHERE IDRistorante = " + ID + ";";
	...
	public ArrayList<PrenotazioniBean> prelevaPrenotazioniRistoranteTraDueOre(RistorantiBean risto, PrenotazioniBean pren) throws Exception{ // Tazza, perch� Vellons non fa mai le cose che mi servono :(
		...
		try {
			...
			Date ora = sdf.parse(pren.getOra());

			String data_str = df.format(range);
